...
def _adaptive_window_sizes(avail_w: int, avail_h: int) -> tuple[int, int, int, int]:
    ...
    - 默认尺寸 min(1280, 可用宽×0.9) × min(920, 可用高×0.9)：历史实际默认
      1030×700 偏小（1080p 仅占半屏），1280×860 下主页面 cover_scale≈1.30，
      封面/信息区随响应式布局同步放大；用户反馈 860 偏扁、比例不协调
      （2026-09-22），默认高提到 920（avail×0.85 → ×0.9），1280×920 下封面区
      与信息区下移余量更足；小屏/高缩放由 min() 自然收窄进屏内。
    ...
    def_h = min(920, max(int(avail_h * 0.9), min_h))
