...
def _adaptive_window_sizes(avail_w: int, avail_h: int, frameless: bool = False) -> tuple[int, int, int, int]:
    ...
    - 最小高取 min(700, 可用高×0.75)：700 可完整容纳主页面内容与设置页底部浮框
      （设计底约 y=696，主页面无滚动区，低于此值底部字段不可达）；×0.75 保证
      125% 缩放（1920x1080 → 逻辑 1536x864）下仍可缩小到 648，不回到锁死 700
      的老问题。（2026-09-22 用户反馈 Windows 原生边框下偏矮导致设置页保存
      按钮显示不全，最小高 650 → 700。）
    - 最小宽 min(850, 可用宽×0.6)：850 为历史值（92ef2437），小屏按比例收窄。
    - 默认尺寸 min(1280, 可用宽×0.9) × min(980, 可用高×0.92)：历史实际默认
      1030×700 偏小（1080p 仅占半屏），1280×920 下主页面 cover_scale≈1.30，
...
def Init_Ui(self: "MyMAinWindow"):
    ...
    self.Ui.pushButton_tips_normal_mode.setToolTip("""<html><head/><body><p><b>正常模式：</b><br/>1）适合海报墙用户。正常模式将联网刮削视频字段信息，并执行翻译字段信息，移动和重命名视频文件及文件夹，下载图片、剧照、预告片，添加字幕、4K水印等一系列自动化操作<br/>2）刮削目录请在「设置→刮削目录」-「待刮削视频目录」中设置<br/>3）刮削网站请在「设置→刮削网站」中设置。部分网站需要代理访问，可在「设置→网络」中设置代理和走代理网站。你可以点击左侧的「检测网络」查看网络连通性<br/>\
    ...
    self.Ui.pushButton_tips_update_mode.setToolTip("""<html><head/><body><p><b>更新模式：</b><br/>1，适合视频已经归类好的情况。更新模式将在不改动文件位置结构的前提下重新刮削更新一些信息<br/>2，更新规则在下面的「更新模式规则中」定义：<br/>-1）如果只更新视频文件名，请选择「只更新C」，视频文件名命名规则请到「设置→命名」中设置<br/>-2）如果要更新视频所在的目录名，请选择「更新B和C」；如果要更新视频目录的上层目录，请勾选「同时更新A目录」<br/>-3），如果要在视频目录为视频再创建一级目录，请选择「创建D目录」<br/>\
        3，更新模式将会对「待刮削视频目录」下的所有视频进行联网刮削和更新。<br/>4，当有部分内容没有更新成功，下次想只刮削这些内容时，请选择「读取模式」，同时勾选「本地没有nfo的文件，重新刮削（按正常模式规则）」，它将查询并读取所有视频本地的 nfo 文件（不联网），当没有 nfo 文件时，则会自动进行联网刮削<br/>5，当部分内容确实无法刮削时，你可以到「软件日志」页面，点击「失败」按钮，点击左下角的保存按钮，就可以把失败列表保存到本地，然后可以手动查看和处理这些视频信息。</p></body></html>""")
    ...
    self.Ui.pushButton_tips_read_mode.setToolTip("""<html><head/><body><p><b>读取模式：</b><br/>\
        ...
        -1）勾选「允许更新 nfo 文件」。<br/>\
        软件将按照「设置→命名」的「Emby视频标题」、「设置→翻译」、「设置→NFO」等的设置项，利用本地 nfo 更新 nfo 信息。<br/>\
...
def Init_Singal(self: "MyMAinWindow"):
    ...
    self.Ui.pushButton_test_javstash.clicked.connect(self.pushButton_test_javstash_clicked)  # [JavStash]
    self.Ui.pushButton_test_stashdb.clicked.connect(self.pushButton_test_stashdb_clicked)  # [StashDB]
    ...
    self.set_javstash_status.connect(self.Ui.label_javstash_test_result.setText)  # [JavStash]
    self.set_stashdb_status.connect(self.Ui.label_stashdb_test_result.setText)  # [StashDB]
