...
def parse_javstash_response(
    res_data: dict | None,
    error: str | None = None,
    status_code: int | None = None,
    service_name: str = "JavStash",
) -> tuple[bool, str]:
    """
    ...
    """
    if error:
        return False, f"❌ {service_name} 连接失败: {error}"
    ...
    if res_data:
        ...
        if isinstance(res_data, dict) and "errors" in res_data:
            ...
            return False, f"❌ {service_name} 密钥无效或请求出错！({error_msg})"
...
def verify_javstash_connection_sync(
    url: str,
    api_key: str,
    proxy: str | None = None,
    timeout: int = 5,
    ...
    try:
        ...
        with opener.open(req, timeout=timeout) as response:
            ...
            return parse_javstash_response(res_data, status_code=response.status, service_name=service_name)

    except urllib.error.HTTPError as e:
        return parse_javstash_response(None, status_code=e.code, service_name=service_name)
    except Exception as e:
        logger.warning("%s connection check failed for %s: %s", service_name, url, e)
        return parse_javstash_response(None, error=str(e), service_name=service_name)


verify_stashbox_connection_sync = verify_javstash_connection_sync
