...
- **打包**：只有字符串动态导入才需 hidden-import（哨兵 `test_build_hidden_imports` 锁）；**所有打包入口（ci 冒烟 Win+Linux / release / build-windows / build-linux）fail-closed，缺 `build/sr_tools` 即硬失败——必须先 `fetch_sr_tools --platform windows|linux` 再 `build.py`**（哨兵 `test_sr_bundling` 逐对锁顺序，ci.yaml 两处冒烟次数必须一致）。周预热 `update-sr-tools.yml` 钉 Python 3.13，Linux Qt/xcb 与 ci.yaml 对齐。SR 缓存一律 `actions/cache@v6`，key 带 `hashFiles('mdcx/core/super_resolution.py')`。发版产物预检覆盖 macOS DMG / Windows EXE / Linux 单文件。**Release**：推纯数字 tag 触发 release.yml；产物名 `MDCx-<版本>-<平台>-<架构>-<完整sha>.<后缀>`；正文取 changelog 当前段；重发=删 release+`git tag -f` 指向新 HEAD+push tag。※已发过版本号可能被用户拍板「删版重发」承载后续修复（不新开版本段）。Actions：runner 标签会整体下线（macos-15-intel 接替 Intel）、setup-uv 无裸主版本浮动标签（须全版本号）。
...
- **媒体服务器 NFO 硬事实（2026-09-23 查 Kodi 官方 wiki v20 页核实）**：`<actor>` 子标签规范只有 name/role/order/thumb，Emby/Jellyfin/Kodi 解析器未知子标签整体跳过，Person 数据模型只有单一名字字段——`<actor><aliases>` 类扩展标签媒体库永远读不到，演员按别名搜索需求只能在生成阶段归一（我方 = actor_database keyword 反查）解决；影片级 `<originaltitle>` 才是全系通吃的"另一写法"通道，影片级 `<aliases>` 是 Jellyfin 扩展 Emby 支持弱。给第三方 NFO 写自定义标签前先确认消费方，我方自存档字段要在 TODO/注释里注明"仅工具链消费"。
