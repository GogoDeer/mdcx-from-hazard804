...
- **主界面「显示封面」勾选框压在简介栏上且点不到**：窗口拉宽放大后勾选框仍按设计坐标固定在页面中部（x 不随缩放），简介/标签行在窗口偏矮时又会上移，两者相撞；勾选框层级还压在简介下划线与缩略图框之下，点击全部被上层吃掉。现勾选框右缘随缩略图框右缘等比右移（右缘始终与缩略图对齐）、纵向贴封面框底，窗口偏矮放不下时自动钳到简介行上方，层级提到缩略图与下划线之上——任何窗口尺寸下都可点击、不再与简介栏重叠
- **主界面工具图标排（编辑 NFO/打开文件夹/播放/右键菜单）放大后不靠右**：整排此前停在设计位置，与右移后的结果树越离越远。现随结果树左缘同步平移（间距不变），窗口缩回时复位
- **信息管理页「批量保存」按钮下用法说明显示不全**：文案加长后固定的 64px 最小高度装不下三行文字被裁断，已加高到 88px
- **默认窗口高度 860 → 920**：窗口偏扁、整体比例不协调；自适应公式同步从「可用高 85%」放宽到 90%（1080p 默认 1280×920，小屏/高缩放仍按比例收进屏内）
