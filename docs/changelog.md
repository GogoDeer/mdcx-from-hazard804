...
- **仓库既有文档全站文案审查**：README 的 Linux 运行文件名改为实际产物名（`MDCx-<版本>-linux-x86_64-<提交号>` 单文件，原先写死 `MDCx`）；INSTALL 系统要求删除无依据的「CentOS 8+」，改为按 GitHub ubuntu-latest 构建的 glibc 基准口径；QUICKSTART「主界面」改为现行导航名「软件界面」；CONFIGURATION 服务器组所属页修正为设置页「演员」标签页（演员管理器对话框内亦可直接改，同一份配置）；USER_GUIDE Emby 场景按钮名对齐实际控件「连接 Emby/Jellyfin」；DEVELOPMENT 水印描述由"9 宫格文字水印"修正为实际的右下角等四角图标图章

- **软件内「使用说明」三处文案过时修正**：「更新模式」描述原有"在不改动文件位置结构的前提下重新整理"自相矛盾，改为与实际行为一致（读取已有 NFO 按更新规则重新整理并移动）；演员库维护的「JavDB 中文名」还写着旧的"影片详情页抓取"实现，改为现行「移动端 API 的 name_zht/name 字段、无需 TMDB API Key」；代理章节补上「直连白名单」「全部走代理」两项说明（网络设置页早有、使用说明漏写）
...
- **Wiki 网络章节与 FAQ 补两条用户向说明**：FAQ 新增 #23 讲清动态域名站点（AVSox/AvMoo/AVHeat）自动跟随换域的机制与「自定义网址优先」例外；网络与代理设置页补「限流不误伤登录态」——429/403/5xx 重试只换指纹、保留 Cookie，被限速时刻不会丢 Cloudflare 通行证或站方会话
- **既有文档全量对齐审查**：开发文档移除 CI「UI 布局检查」职责描述（该检查已移出门禁）、Git 钩子更新为 `.githooks` 入库机制（pre-push 全量自检 + 署名，环境重置后需重配 hooksPath）、测试标记描述与 CI 过滤现状对齐、手动构建补充超分工具拉取前置；配置参考与维基限速 FAQ 补充 429/503 `Retry-After` 自动冷却说明
- **GitHub Wiki 扩展为 13 页分站**：新增安装与运行/刮削基础/模式与刮削策略/支持站点与图源（36 爬虫一览表）/图片下载的封面优化/命名与目录整理/翻译设置/网络与代理设置/Emby 集成与演员管理/进阶技巧 10 页用户使用文档，重写首页与侧边栏分组导航；内容与 FEATURES/CONFIGURATION/USER_GUIDE 已核实事实逐项对齐（含免 CF 通道、字段优先级、Retry-After 冷却、TRAWL 便携服务、演员数据纠错条目）

### 开发

- **本地/CI 自检减负**：推送前自检按改动内容分流——本次推送全部为文档类（`.md`/`.mdx`/`.monkeycode/`/`docs/`/`wiki/`）时只检查空白问题即放行，含代码改动才跑全量 `check`（新分支无基线时保守全量；CI 侧本就有同款分流，钩子与之对齐）；出厂演员库/信息库校验只在对应 xlsx 或校验脚本改动时执行；`check_ui_layout` 移出门禁（结构约束由现有 UI 结构测试锁定，该脚本改为手工诊断）
- **尊重站点 Retry-After 限流头**：异步请求收到 429/503 且响应带 `Retry-After`（秒数或 HTTP 日期）时，按站点要求暂停该站点的所有请求（上限 300 秒，等待发生在限速之前、并发任务共享同一截止点），不再只靠固定速率桶硬撞限流；无该头时行为不变
- **批量保存说明高度回归测试修平台敏感断言**：短文本回落断言由恒定 88 改为「88 与当前宽度换行实测高取大」——Windows runner 字体/DPI 下换行高可超 88 导致 CI 误红，Linux 下仍严格验证钳到下限的语义
- **samestat 同文件判定护栏锁测**：成功移动前的"两路径同文件"判定新增大小+修改时间双护栏后，补正反两条行为锁——dev/ino 相同但元数据不一致必须报同名冲突，全一致才允许免移动，防后续重构悄悄拆掉护栏

- **超分工具缓存键随基准轮换**：打包工作流的 `build/sr_tools` 缓存 key 追加 `hashFiles(mdcx/core/super_resolution.py)` 指纹——actions/cache 同 key 永不覆写、fetch 命中后不复核校验和，固定 key 下升级工具基准后新旧工作流（含每周预热任务）都会静默命中旧缓存打旧版工具；哨兵测试锁定五条工作流的 key 轮换

### 修复

- **信息管理页「批量保存用法说明」框高度只升不降，Windows 上残留大片空白**：按换行行数贴合高度此前只抬最小高度——窄窗/长文本把框撑高后改回短文本，布局不会回收已撑高的当前高度，说明框下方残留空白（Windows 实机与 CI 必现）。现改为按当前宽度换行高钉固定高度（下限 88px，窗口缩放时随换行数重算），长短文本切换即时贴合
- **信息管理页「用新内容替换所选列」把简介换行压成一行**（议题 #138）：批量替换直接写 `plainTextEdit.toPlainText()`（会拍平换行），与单条编辑路径（保存前把换行转成 `<br>`）行为不一致——对已有 `<br>` 的简介做替换后换行全部消失，Emby 里整段简介变成一坨。现批量替换出口复用单条编辑同一归一化（换行转 `<br>`），并顺带补齐 `\r\n` 换行归一（Windows 记事本风格换行此前被当普通字符混进简介）
- **软件界面结果树高度不随窗口拉伸，底部留大片空白**：树高此前固定设计值 563px，窗口拉高后树底与窗缘间空出 200px+ 不可用。现树高跟随页面高度（保留设计底距 19px），最大化后列表可视行数同步增加
- **信息管理页「批量保存」用法说明被裁且与按钮间空白大**：说明文字高度写死 88px 且垂直居中——窄窗口/高 DPI 下文字换行到 4 行超出被裁，行数少时上下又留大片空白。现文字改顶部对齐、高度按当前宽度实际换行行数贴合（88px 保底），两种窗口尺寸下都完整显示、无多余空白
- **编辑 NFO 面板：简介/原简介/标签/原标题框高度跟随内容、完整换行**：四个多行框此前固定高（简介/原简介 150、标签 100），长内容只能在框内滚动看不全；「原标题」更是单行输入框，日文长标题直接截断。现四框全部按内容自适应增高（短内容保持原设计高，长内容按换行行数撑开，高度随输入即时重算），面板拉宽/缩窄时高度同步重排；内容超出窗口时外层滚动条承接，保存/关闭按钮仍钉底不被推走
- **主界面右侧对齐体系（勾选框压简介栏点不到 / 工具图标与下划线不靠右 / 信息区左右平衡）**：窗口拉宽后多处元素仍停在设计坐标——①「显示封面」勾选框按设计坐标固定在页面中部，简介/标签行在窗口偏矮时又会上移，两者相撞，且勾选框层级压在简介下划线与缩略图框之下、点击全被上层吃掉；②工具图标排（编辑 NFO/打开文件夹/播放/右键菜单）停在最左不动；③简介/标签/时长/系列/发行等下划线右缘封顶在缩略图右缘，右侧留一大片空白；④封面/缩略图框不随窗口拉伸。现引入统一右界（结果树左缘-13 与缩略图右缘取大），下划线、勾选框、工具图标排、缩略图框右缘全部对齐到这条线；封面/缩略图组参照媒体服务器详情页左右双平衡——封面左缘紧贴「封面:」标签列，两框等高且各自锁定源图比例（竖版封面 156:220、横版缩略 328:220），宽度反推高度、零变形；勾选框层级提到缩略图之上、纵向贴封面框底，任何窗口尺寸都可点击；简介起整块信息区改滚动容器（滚动条始终可见），封面可按窗口放大而不把下方字段挤出视野，简介自动换行完整显示
- **刮削模式「允许更新 nfo 文件」说明路径写错**：「Emby视频标题」在「设置→命名」，说明里写成「下方」会误导。已改为「设置→命名」
- **首次显示后个别控件短暂错位后自愈（Windows 原生边框）**：启动首帧设置页浮框等绝对定位控件可能滞留旧几何、隔一会儿才恢复——与议题 #78「原生边框 resize 时序错过布局更新」同族。现默认尺寸应用后在事件循环空转一轮后强制补同步一次（幂等），根除首帧错位
- **默认窗口高度 920 → 980、最小高 650 → 700**：窗口偏扁、比例不协调（隐藏边框模式反馈）；Windows 原生边框模式下标题栏与系统边框额外占用屏幕高度，默认/最小窗口偏矮时设置页底部「当前配置/另存为/恢复默认/保存」浮框显示不全（原生边框模式反馈）。自适应公式同步放宽：默认高从「可用高 90%」提到 92%、上限 980（1080p 无缩放下默认 1280×956），最小高从 650 提到 700（125% 缩放下仍可缩到 648，不回到"锁死无法缩小"的老问题）
- **信息管理页「批量保存」按钮下用法说明显示不全**：文案加长后固定的 64px 最小高度装不下三行文字被裁断，已加高到 88px
...
- **NFO 写入失败仍被记为刮削成功，断点续刮永久跳过缺 NFO 的影片**：写 NFO 的失败返回值此前被忽略——磁盘满/权限不足时影片进入成功清单，续刮按成功记录跳过，而磁盘上始终没有 NFO。现失败写入记录错误并从成功清单剔除、计入刮削失败统计，下次续刮自动重试

- **`PREFER_NFO` 本地优先合并读错文件**：合并旧 NFO 按"源视频文件名"找 NFO 路径，源文件名与目标命名不一致时读不到真正的旧 NFO——合并静默失效或读到同目录无关文件。现直接从旧 NFO 实际路径读取

- **影片移动失败留下 NFO/字幕孤悬的割裂目录**：收尾阶段先写新 NFO、移动伴生文件，再移动视频本体；一旦本体移动失败（成功目录同名冲突、跨盘 rename 失败）没有任何回滚，出现"旧目录 NFO 已搬走、新目录只有 NFO 没有视频"。现调整为视频本体先移动，成功之后才写 NFO 与伴生文件

- **同站点多文件并发刮削互相污染爬虫状态（base_url / 镜像轮换错位）**：爬虫为站点级共享单例，`base_url`、JavDB 镜像索引、域名轮换计数器都是实例属性——两个文件同时刮同一站点时互相覆盖，表现为 A 的请求打到 B 刚切换的镜像、镜像轮换被并发打乱后固定撞同一坏节点。全部改为按刮削任务隔离（ContextVar），站点级连接资源不受影响

- **DMM 等慢站点被批次超时提前斩杀、降级原因误标"请求超时"**：批次级超时固定 60 秒，小于站点自身（超时 10 秒 × 重试 4 次 × 双通道）的 80 秒窗口——站点自身的镜像重试与降级路径还没走完就被外层掐断，排障时又被"请求超时"字样误导。批次超时改为随网络配置自动缩放（`timeout×(retry+1)×4`，下限 60 秒）

- **批次内个别站点超时连带丢弃同批已完成站点的结果**：批次组超时此前整体丢弃 `gather` 的全部结果——已正常返回的站点数据一并作废，只剩一个 TimeoutError。现仅未完成的站点记超时，已完成结果原样保留进合并流程

- **动态解析域名的站点从未走过动态通道**：入口域名解析的第一步就拿 `base_url` 的非空回退默认域短路，动态域名获取（含每日缓存更新）成为永远执行不到的死代码——站点换域后只能手动改配置。现仅用户自定义 URL 短路，未自定义时正常走"动态获取 → 默认域兜底"

- **站点返回 429/403/5xx 时整个会话被连根重建，Cookie 恰好在被判限速时清空**：可重试状态码的处理与连接层故障共用同一"拆连接池"动作——Cookie 罐、TLS 会话复用全部丢弃，cf_clearance 一类凭据在 Cloudflare 拦截时刻恰好被抹掉，重试要重新过人机验证，与 Retry-After 冷却互相抵消。现状态码失败只换用下一个浏览器指纹，连接池与 Cookie 保留；拆池仅保留给真正的传输层故障

- **演员映射表缺目标语言字段时 None 混入演员列表**：按语言映射演员名时映射缺失返回 None 直接进结果列表，NFO 演员条目出现空值。现缺失时回落演员原名

- **并发细节加固**：首次使用超分工具时多张海报并发触发同工具重复下载/互相覆盖解压，现按工具加锁串行化；水印 JPEG 编码落盘改为线程执行不再阻塞事件循环；移动成功判定在 inode 一致性之上叠加大小与修改时间双护栏防极端误判

### 优化

- **演员管理器下拉与数据源标注措辞通俗化**（借鉴社区 fork z291173301/MDCx 的提议方向）：「更新模式」组里「更新所有演员详情（不含头像/影片数）」动词统一为「重新获取所有演员详情…」，与同组「重新获取所有演员简介/头像」一致，减少一处"更新/重新获取"两词并存的困惑；头像数据源「本地头像」改「本地演员照片目录」，与设置项「演员照片目录」对应（2026-09-23）

- **封面补图链路接上海报超分**：超分收尾钩子此前只作用于刮削主流程，「软件工具 → 封面补图」下载落盘的低清海报不会补清。现补图在海报定稿后走同一钩子——同开关/阈值/体积护栏自动判定，先超分再加水印，失败保持原图；裁剪落盘的海报同样受益。DMM 官方直链分支本就是高清源，尺寸超阈值自然跳过。补图钩子接入与行为测试两条（poster_ok 时触发且先于水印、缺海报不触发）（2026-09-23 用户需求）

- **隐藏边框（美观样式）默认窗口更紧凑**：隐藏边框无系统标题栏做视觉兜底，与原生边框共用同一默认尺寸时突兀感明显（1080p 高占屏 91%）。现该模式默认尺寸单独收一档（约 ×0.9），收回不低于最初设计 1030×700——1080p 默认 1152×860（占屏高 82%）；原生边框维持现状，最小尺寸与用户调整过的大小均不受影响，新默认仅作用于首次启动（2026-09-23 用户反馈）
