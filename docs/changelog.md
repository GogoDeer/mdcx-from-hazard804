...
- **#26 新增海报超分增强（实验性，默认关闭）**：老片/下架图常只剩 147x200 级低清封面，此前"高清升级"只能靠站内换源，全网无高清就没办法。现支持对最终落盘海报做 AI 超分辨率放大（Real-ESRGAN 4x / waifu2x 2x 两档预设，最长边低于阈值才触发）：工具二进制首次使用时从 GitHub Release 自动下载（约 30-60MB，仅一次），全程失败静默保持原图、不影响刮削结果。配置项 `poster_sr_enabled`（默认 false）/ `poster_sr_preset` / `poster_sr_max_dim`，设置页图形开关后续版本提供
