...
- **主界面右侧对齐体系（勾选框压简介栏点不到 / 工具图标与下划线不靠右 / 封面右侧留白）**：窗口拉宽后多处右侧元素仍停在设计坐标——①「显示封面」勾选框按设计坐标固定在页面中部，简介/标签行在窗口偏矮时又会上移，两者相撞，且勾选框层级压在简介下划线与缩略图框之下、点击全被上层吃掉；②工具图标排（编辑 NFO/打开文件夹/播放/右键菜单）停在最左不动；③简介/标签/时长/系列/发行等下划线右缘封顶在缩略图右缘，右侧留一大片空白；④封面/缩略图框同样不跟随。现引入统一右界（结果树左缘-13 与缩略图右缘取大）：封面/缩略图框、尺寸文字、勾选框、简介/标签与右列下划线、工具图标排的右缘全部对齐到这条线；勾选框层级提到缩略图之上、纵向贴封面框底且矮窗口时自动钳到简介行上方，任何窗口尺寸都可点击、互不重叠；封面缩放同时受高度约束（宽而矮的窗口封面自动少放大），信息区首行恒在封面框底下 50px、底行不溢出
