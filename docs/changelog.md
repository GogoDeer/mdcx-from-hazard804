...
### 新增
- **影片搜刮一键还原与 AI 诊断系统**：
  - **右键菜单单选入口（快捷键 `R`）**：在刮削结果列表单选某部影片右键（或直接按 `R`），提供【还原影片并复制日志】功能；多选或未选中影片时安全隐藏，防止误操作。
  - **环境与伴随文件完全复原**：主视频安全移回原路径并恢复原文件名；软/硬链接模式下仅删除输出端链接、绝不触碰原物理文件；伴随转移的字幕（如 `.zh.srt`、`.cht.ass` 等）、种子（`.torrent`）、`.bif` 等原路移回原目录。
  - **刮削垃圾清理与空目录回收**：自动清理该影片本次刮削生成的 NFO、海报、剧照、缩略图、`extrafanart`、`.actors` 等文件；输出目录若变为空目录则自动安全回收移除。
  - **刮削历史注销**：自动从内存缓存与磁盘 `userdata/success.txt` 中注销该影片，后续重新刮削不会被当成已成功跳过。
  - **AI 专属 Markdown 诊断报告与剪贴板集成**：自动汇编结构化报告并写入系统剪切板（可直接在聊天框 Ctrl+V 发送给 AI 排查）；报告显著标明 **MDCx 配置文件路径**（`manager.path`）与 **全量运行日志文件路径**（`Flags.log_txt`），并自动同步持久化快照至 `Log/ai_reports/<时间>_<番号>_diagnose.md`。
  - **一键快捷重搜**：还原成功弹窗提供【重新指定番号】快捷按钮，一键唤起番号输入框重新发起刮削。
