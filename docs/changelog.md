...
- **演员管理器下拉与数据源标注措辞通俗化**「更新模式」组里「更新所有演员详情（不含头像/影片数）」动词统一为「重新获取所有演员详情…」，与同组「重新获取所有演员简介/头像」一致，减少一处"更新/重新获取"两词并存的困惑；头像数据源「本地头像」改「本地演员照片目录」，与设置项「演员照片目录」对应

- **封面补图链路接上海报超分**：超分收尾钩子此前只作用于刮削主流程，「软件工具 → 封面补图」下载落盘的低清海报不会补清。现补图在海报定稿后走同一钩子——同开关/阈值/体积护栏自动判定，先超分再加水印，失败保持原图；裁剪落盘的海报同样受益。DMM 官方直链分支本就是高清源，尺寸超阈值自然跳过。补图钩子接入与行为测试两条（poster_ok 时触发且先于水印、缺海报不触发）

- **隐藏边框（美观样式）默认窗口更紧凑**：隐藏边框无系统标题栏做视觉兜底，与原生边框共用同一默认尺寸时突兀感明显（1080p 高占屏 91%）。现该模式默认尺寸单独收一档（约 ×0.9），收回不低于最初设计 1030×700——1080p 默认 1152×860（占屏高 82%）；原生边框维持现状，最小尺寸与用户调整过的大小均不受影响，新默认仅作用于首次启动
