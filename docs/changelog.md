...
- **刮削状态缓存三增强**：①站点明确回复「查无此番号」的刮削失败写入 7 天负面缓存——断点续刮期内直接跳过、不占自动重试预算、不进恢复队列，到期自动放行重刮（站点可能后来收录），改过文件（如修正番号笔误）立即允许再试；②缓存记录写入解析代版本——解析/数据修正类修复的发版发布后，旧代「已完成」记录自动放行重刮，历史记录随下次刮削自动追平，不再需要手动清缓存；③「刮削缓存管理」新增「检测缺字段记录并重置」——列出刮削成功但标题/演员/发行/时长为空的记录（站点改版期刮到空数据的典型特征），确认后一键重置下次重刮补全

- **#26 海报超分辨率增强（实验性）**：老片/下架图常只剩 147x200 级低清封面，此前"高清升级"只能靠站内换高清源。现对落盘海报做 AI 超分放大（Real-ESRGAN 4x / waifu2x 2x 两档预设，仅当海报最长边低于阈值（默认 800px）时触发，失败静默保持原图）；**开关默认开启**（设置 → 下载 → 下载高清图组，与 Amazon 高清图同组）。Windows / Linux 打包版已内置工具；macOS 打包版与源码运行首次使用时从官方 GitHub Release 下载（约 30-60MB，仅一次，落盘前校验 sha256）。预设与阈值可经配置 `poster_sr_preset` / `poster_sr_max_dim` 细调。「软件工具 → 封面补图」落盘的海报与裁剪落盘海报同样走此钩子（定稿后先超分再加水印）

- **#21 图床失败自动冷却**：某图床连续返回服务器侧错误（403/429/5xx/Cloudflare 拦截）时，此前每次刮削都重新逐一撞一遍死图床，拖慢整批。现 60 秒窗口内同图床累计 3 次服务器侧失败即冷却 2 分钟，期内该图床候选直接跳过、自动改用备用图源；冷却状态持久化到磁盘，程序重启/断点续刮后依然生效；成功即解除。本地网络问题与 404 未收录不计入

- **#30 DMM 图床链接探测提速**：图片候选存在性校验此前每次完整下载整图（一批 15+ 候选各 3-4 秒，站点吞吐成为批刮瓶颈）。现改为 `Range: bytes=0-1023` 流式探测——只传 1KB 即判定存在与总大小，占位图阈值逻辑不变；个别服务器不回大小头时自动回退一次完整下载

- **尊重站点 Retry-After 限流头**：收到 429/503 且响应带 `Retry-After`（秒数或 HTTP 日期）时，按站点要求暂停对该站的所有请求（上限 300 秒，并发任务共享同一截止点），不再只靠固定速率桶硬撞限流；无该头时行为不变

- **窗口尺寸按屏幕自适应**：启动尺寸不再固定 1030×700——原生边框模式按可用区放大（1080p 默认 1280×956），封面与信息区同比例放大，修复了大屏上窗口偏扁、比例不协调的问题；边框隐藏（美观样式）模式自带视觉兜底，默认单独收一档至约 1152×860。最小尺寸全平台统一（1080p 最小高 550→700，解决底部字段裁到够不着）。此前窗口偏矮时导航栏左下角状态文字被裁，现状态区保证完整可见；启动首帧个别控件短暂错位后自愈的问题（Windows 原生边框时序）也已根治，补一次幂等布局同步

- **GFriends 头像多候选名匹配**：头像查找此前拿演员"当前一个写法"去撞头像库，撞不上即弃——中日文写法差异、演员库登记的别名（繁/简/日文/keyword）全都命中不了。现按演员数据库反查整行组装候选名依次尝试，任一命中即取头像；头像库索引归一比对改为构建一次全程复用（此前每位演员每次查找都全表逐条归一万级文件名），批量获取头像同步提速

- **演员管理器界面措辞通俗化**：「获取数据」下拉的「更新所有演员详情（不含头像/影片数）」动词统一为「重新获取…」（与同组简介/头像项一致）；头像数据源「本地头像」正名「本地演员照片目录」

### 修复

- **NFO 数据完整性链路**：写 NFO 失败此前被静默记为成功，磁盘满/权限不足时文件进成功清单、断点续刮永久跳过缺 NFO 的影片——现失败剔除并按刮削失败统计，续刮自动重试；`PREFER_NFO` 本地优先合并此前按"源视频文件名"找旧 NFO，源文件名与目标命名不一致时读错读到无关文件——现从旧 NFO 实际路径读取；收尾阶段先写 NFO 再移视频本体，本体移动失败会留下"NFO 已搬走、视频还留旧目录"的割裂态——现调整为本体先移动、成功之后才写 NFO 与字幕等伴生文件

- **同站点并发刮削互相污染爬虫状态**：爬虫为站点级共享实例，`base_url`、镜像索引、域名轮换计数器互相覆盖——A 的请求打到 B 刚切换的镜像、轮换被打乱后反复撞同一坏节点。全部改为按刮削任务隔离，站点级连接资源不受影响

- **批次超时与结果保留**：批次级超时此前固定 60 秒，小于站点自身重试窗口——站点自己的镜像重试/降级路径还没走完就被外面掐断，降级原因还被误标"请求超时"；现随网络配置自动缩放并按站点自适应，站点自身窗口永远优先。批次内个别站点超时此前连带丢弃整批结果——已正常返回的站点数据一并作废作废重来，现仅未完成站点记超时、已完成结果原样保留

- **动态域名站点自动跟随换域恢复生效**：AVSox/AvMoo/AVHeat 等站入口域名解析第一步就被默认兜底域短路，"每日动态解析新域名"成为永远执行不到的死代码。现未自定义站点地址时正常走「动态解析 → 默认域兜底」；自定义地址优先，站点真换域后最迟次日自动跟上

- **站点限流不再清空登录会话**：站点返回 429/403/5xx 时，处理与连接层故障共用同一"拆连接池"动作——Cookie 罐、TLS 会话复用整个丢弃，cf_clearance 一类凭据在 Cloudflare 拦截时刻恰好被抹掉，重试要重新过人机验证，与刚加上的 Retry-After 冷却互相抵消。现状态码失败只换用下一个浏览器指纹，连接池与 Cookie 保留；拆池只留给真正的传输层故障

- **主界面响应式布局体系**：窗口拉宽后大量元素停在设计坐标——「显示封面」勾选框压简介栏且被上层吃掉点击、工具图标排钉在左侧、下划线/框体各自封顶留大片空白、结果树宽高固定不动。现引入统一右界对齐体系（下划线、勾选框层级、图标排、框体右缘一线对齐）；封面/缩略图框按窗口等比拉伸且各自锁定源图比例零变形；结果树高度跟随页面高度、宽度随封面缩放系数等比（最大化时与缩略图间距从 ~280px 收窄到 ~62px）；简介起整块信息区改滚动容器，封面放大不再挤掉下方字段

- **编辑 NFO 面板多行框随内容自适应**：简介/原简介/标签/原标题四个框此前固定高，长内容框内滚动看不全，原标题还是单行截断。现四框按内容增高、短内容保持设计高，面板缩放时高度实时重排；内容超出窗口由外层滚动条承接，保存/关闭按钮始终钉底

- **信息管理页「批量保存」与「替换所选列」**：用法说明框此前高度只增不减，长文/窄窗把框撑高后切回短文，下方残留大片空白（Windows 与 CI 必现）——现按当前宽度实际换行行数钉定高度（88px 保底），增减即时贴合；「用新内容替换所选列」此前把简介换行压成一行（Emby 里整段变一坨）——现与单条编辑同一归一化（换行转 `<br>`，兼容 CRLF）

- **演员信息数据链路三连修**：数据清洗范围收窄回仅清除 minnano 占位文案——此前误把 `<br>`/换行/`=====` 段标题当噪声压成逗号，Jellyfin/Emby 端演员简介从分行列表退化成一段话，现已保留；「全量获取演员资料」大量「表格列数不匹配」告警、生日/出生地丢失——wiki 日文信息框数据格带对齐样式解析不到，解析改逐行配对天然免疫样式/跨列差异，并识别「出生地」标签（旧仅认「出身地」）；演员详情窗把简介里的 `<br>` 当字面文本显示——现展示层转换为真实换行，写回不动。另：「跳过 N 个不在所选库中出演(Actor) 的人员」为预期行为——Emby 接口不支持按角色过滤，按库内演职员表角色=演员取交集

- **演员管理器取数中途关窗线程安全**：正在取数时关闭演员管理器会把还在跑的后台线程连窗口一起拆掉——Windows 上整进程直接中止。现关闭前取消并等待全部工作线程，等不及的线程脱离窗口自行收尾，主程序保持运行；「停止获取」后立即重开被废弃任务回调整体作废，进度条与列表不再被旧任务数据覆盖

- **切页后编辑 NFO 面板不再盖住功能页**：切到「软件日志/软件工具」等页时仍在新页上挡住右侧内容，且挡住点不到。现切页时自动暂隐面板、切回自动恢复，有未保存改动先弹保存/丢弃/确认，取消留在原页继续编辑；面板左缘贴导航栏最左、挡住按钮的观感一并解决

- **#179 刮削缓存管理失败列表列宽自适应**：小窗横向滚动条与最大化后右侧大片空白并存，列宽固定、刷新后更失控。现文件名/最后错误两列随视口分摊、其余列按内容收缩，列宽永远填满且不超出视口

- **#174 网络检测后 theporndb 下拉无状态点**：该 Token 检测项此前漏挂站点归属，网络设置和刮削网站两处显示空状态，现补上（有 Token 且通过=绿、未填=黄）

- **边界与并发加固**：演员映射表缺目标语言字段时回落演员原名——此前 None 直接混进演员列表污染 NFO；首次使用超分工具时多张海报并发触发重复下载/解压互相覆盖，现按工具加锁；水印 JPEG 编码落盘改线程异步执行不再阻塞事件循环；成功移动前"两文件是同一文件"的 inode 判定叠加大小+修改时间双护栏

### 文档

- **站内文档与使用说明全站对齐审查**：README/INSTALL/QUICKSTART/CONFIGURATION/USER_GUIDE/DEVELOPMENT 与软件内使用说明、配置参考全部按现行行为校准——Linux 运行改实际产物名与 glibc 基线口径，「更新模式」「读取模式」「JavDB 中文名」（改移动端 API、无需 TMDB Key）等过时描述改写，代理章节补「直连白名单/全部走代理」，删除旧浏览器模式遗留的孤儿说明，「Emby视频标题」路径等设置路径纠错；开发文档与 Git 钩子、CI 现状同步

- **GitHub Wiki 重组为 13 页分类导航**：新增安装与运行/刮削基础/模式与刮削策略/支持站点与图源（36 爬虫一览）/命名与目录整理/翻译设置/网络与代理设置/Emby 集成与演员管理/进阶技巧等 10 页分类文档，重写首页与侧边栏；FAQ 补动态域名跟随机制、维基双桶限速、限流冷却与「限流不误伤登录态」等条目；软件内 UI 文案与文档术语全面统一（NFO 合并策略中文下拉名、设置页「高级」、默认源列表、启动自检描述五项）

### 开发

- **本地/CI 自检减负**：推送前自检按改动内容分流——纯文档推送只查空白即放行（与 CI docs-only 分类同口径），含代码才跑全量 `check`；出厂演员库/信息库校验仅在对应 xlsx 或校验改动时执行；`check_ui_layout` 移出门禁改手工诊断

- **爬虫测试 recording 回放框架**：新增请求录像回放架子（tests/crawlers/recording.py）——一次真实抓库存为 JSON 录像，此后 CI/日常测试把回放客户端注入爬虫离线重跑整条采集链，替代「手写页面夹具与真实站点漂移」和「测试打真站不稳定」两代老路；镜像轮询站点录像与域名解耦，未录请求硬报错防假绿。已带 xcity（404 回退链）/ avmoo（JSON API 链）/ javdb_api（镜像轮换链）三条整链离线样例，站点改版后重录即可回归

- **超分工具缓存键随基准轮换**：打包工作流 `build/sr_tools` 缓存 key 叠加工具基准文件指纹，基准更新缓存自动失效、sha256 不匹配直接构建失败，防止升级后静默命中旧缓存打旧版工具；缓存动作统一为 `actions/cache@v6`

- **打包工作流缺口补齐**：超分工具周预热钉 Python 3.13，Linux 系统库与 CI 对齐（缺 Qt/xcb 时预热会在 import PyQt6 直接失败）；CI 增加 Linux PyInstaller 冒烟，把省略图标与超分内嵌参数前置到 PR/主干；发版产物预检覆盖 macOS / Windows / Linux，缺文件或空文件在上传前失败
