...
- **主界面右侧对齐体系（勾选框压简介栏点不到 / 工具图标与下划线不靠右 / 信息区左右平衡）**：窗口拉宽后多处元素仍停在设计坐标——①「显示封面」勾选框按设计坐标固定在页面中部，简介/标签行在窗口偏矮时又会上移，两者相撞，且勾选框层级压在简介下划线与缩略图框之下、点击全被上层吃掉；②工具图标排（编辑 NFO/打开文件夹/播放/右键菜单）停在最左不动；③简介/标签/时长/系列/发行等下划线右缘封顶在缩略图右缘，右侧留一大片空白；④封面/缩略图框不随窗口拉伸。现引入统一右界（结果树左缘-13 与缩略图右缘取大），下划线、勾选框、工具图标排、缩略图框右缘全部对齐到这条线；封面/缩略图组参照媒体服务器详情页左右双平衡——封面左缘紧贴「封面:」标签列（不再整体右移留左侧空洞），缩略图宽度自适应拉伸填满到右界；缩略图显示改「等比裁剪填充」（类似播放器封面画面），加宽后剧照裁上下不变形；勾选框层级提到缩略图之上、纵向贴封面框底且矮窗口时自动钳到简介行上方，任何窗口尺寸都可点击、互不重叠；封面缩放同时受高度约束（宽而矮的窗口封面自动少放大），信息区首行恒在封面框底下 50px、底行不溢出
