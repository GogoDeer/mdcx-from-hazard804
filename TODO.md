...
### 3. 演员别名补全 ❌（2026-09-23 取消）
- 取消理由：三条真实需求全部被现成路径提前满足——①库内/NFO 写法统一 = actor_database keyword 反查索引 + `map_actor_names` 已在位；②别名匹配（GFriends 头像）= #15 直接消费反查行，工具链内自足；③媒体库按别名搜索 = Emby/Jellyfin/Kodi 演员仅认 `<actor>` 的 name/role/order/thumb、Person 模型单一名字，`<actor><aliases>` 无消费方，NFO 自存档是死备份、撑不起立项
- 标签声明式配置已拆出为 #34，继续保留
...
### 15. GFriends 候选名列表匹配 ✅（2026-09-23 实现）
- **价值：中**　**难度：低**（实耗半天）
- `gfriends_find_actor` 支持候选名列表（按序尝试首个命中即返回，单名调用旧语义保留）；`_gfriends_candidate_names` 从演员数据库反查整行组装（原名/繁/日/简/keyword 别名，保序去重，未命中或库异常退化为 [原名]）
- 慢点同修：`GFriendsIndex`（dict 子类惰建归一化查找表）——`_expand` 两条路径统一包装，键侧归一从"每演员每次全表"降为构建一次复用，json 缓存序列化形态不变
- 未采用 `ActorInfo.aliases` 字段方案：候选在消费点从现成反查行组装，避免给数据模型加只为一处消费服务的字段
...
- **价值：中**　**难度：高**（1-2 周）
- 演员作为一等实体：资料卡（头像+姓名+别名+作品数）、按罩杯/身高/年龄/作品数排序、跨语言别名归一（依赖 actor_database 反查索引，#15 已示范同款用法）
...
- **2026-09-23 三仓增量复查（javdb-cli/OpenAver/amane 均有当日推送）**：
  ...
  - 结论：三仓近三日**无需码增量**；后续增量扫描建议冻结至下轮站点改版潮。2026-09-23 进度更新：#1 已完成；#15 以"候选名取自数据库反查行"当天完成，#3 因需求被现成路径全覆盖而取消（标签配置拆 #34）；实装顺序改为 #22 → #11
