...
async def test_missing_api_key_raises(mock_client):
    ...
    with pytest.raises(CrawlerException, match="请在设置中配置 StashAPI 令牌"):
...
async def test_graphql_error_response_raises(mock_client):
    ...
    with pytest.raises(CrawlerException, match="field not found"):
        await crawler._post_graphql(ctx, "query { }", {})
...
async def test_graphql_http_error_raises(mock_client):
    ...
    with pytest.raises(CrawlerException, match="GraphQL 请求失败"):
        await crawler._post_graphql(ctx, "query { }", {})
...
async def test_run_with_number(mock_client, stars358_scene):
    manager.config.javstash_api_key = "test_key"
    crawler = StashGraphQLCrawler(client=mock_client)
    crawler._post_graphql = AsyncMock(return_value={"queryScenes": {"scenes": [stars358_scene]}})
...
async def test_run_not_found(mock_client):
    manager.config.javstash_api_key = "test_key"
    crawler = StashGraphQLCrawler(client=mock_client)
    crawler._post_graphql = AsyncMock(return_value={"queryScenes": {"scenes": []}})

    ctx = Context(input=CrawlerInput.empty())
    ctx.input.number = "NOT-FOUND"

    with pytest.raises(CrawlerException, match="未找到匹配场景"):
...
async def test_run_with_hash_lookup(mock_client, stars358_scene, monkeypatch):
    ...
    manager.config.javstash_api_key = "test_key"
    crawler = StashGraphQLCrawler(client=mock_client)
    crawler._post_graphql = AsyncMock(return_value={"findScenesBySceneFingerprints": [[stars358_scene]]})
    ...
    crawler._post_graphql.assert_called_once()
    args = crawler._post_graphql.call_args[0]
    assert args[2] == {"fingerprints": [[{"algorithm": "OSHASH", "hash": "fake_oshash_12345"}]]}


@pytest.mark.asyncio
async def test_run_with_phash_lookup(mock_client, stars358_scene, monkeypatch):
    from mdcx.utils.phash import clear_cached_scenes

    clear_cached_scenes()
    manager.config.javstash_api_key = "test_key"
    crawler = StashGraphQLCrawler(client=mock_client)
    # OSHASH returns empty match, PHASH returns stars358
    crawler._post_graphql = AsyncMock(return_value={"findScenesBySceneFingerprints": [[], [stars358_scene]]})
    monkeypatch.setattr("oshash.oshash", lambda p: "fake_oshash_12345")
    ...
    assert args[2] == {
        "fingerprints": [
            [{"algorithm": "OSHASH", "hash": "fake_oshash_12345"}],
            [{"algorithm": "PHASH", "hash": "f19ea8f1c0e89983"}],
        ]
    }
...
async def test_file_scraper_integration_with_javstash(mock_client, stars358_scene):
    config = Config()
    config.javstash_api_key = "test_key"
    for field in CrawlerResultFields:
        config.get_field_config(field).site_prority.append(Website.JAVSTASH)
    provider = CrawlerProvider(config, mock_client)
...
def test_graphql_query_constants_structure():
    """Verify that GraphQL queries strictly adhere to Stash-box schema and avoid Stash-app fields."""
    # 1. Number search MUST use queryScenes(input: SceneQueryInput!) and NOT findScenes
    assert "queryScenes" in StashGraphQLCrawler.FIND_BY_NUMBER_QUERY
    assert "findScenes(" not in StashGraphQLCrawler.FIND_BY_NUMBER_QUERY
    assert "$input: SceneQueryInput!" in StashGraphQLCrawler.FIND_BY_NUMBER_QUERY

    # 2. Hash search MUST use findScenesBySceneFingerprints with 2D list FingerprintQueryInput
    assert "findScenesBySceneFingerprints" in StashGraphQLCrawler.FIND_BY_HASH_QUERY
    assert "$fingerprints: [[FingerprintQueryInput!]!]!" in StashGraphQLCrawler.FIND_BY_HASH_QUERY
    assert "checksum" not in StashGraphQLCrawler.FIND_BY_HASH_QUERY

    # 3. ID search MUST use findScene(id: ID!)
    assert "findScene(id: $id)" in StashGraphQLCrawler.FIND_BY_ID_QUERY
    assert "$id: ID!" in StashGraphQLCrawler.FIND_BY_ID_QUERY

    # 4. All queries must include SceneFragment
    for query in (
        StashGraphQLCrawler.FIND_BY_NUMBER_QUERY,
        StashGraphQLCrawler.FIND_BY_HASH_QUERY,
        StashGraphQLCrawler.FIND_BY_ID_QUERY,
    ):
        assert "...SceneFragment" in query
        assert "fragment SceneFragment on Scene" in query


@pytest.mark.asyncio
async def test_graphql_error_detail_propagation(mock_client):
    """Verify that HTTP 422/400 validation error descriptions are preserved in the exception."""
    manager.config.javstash_api_key = "test_key"
    crawler = StashGraphQLCrawler(client=mock_client)
    mock_client.post_json.return_value = (
        None,
        'POST https://javstash.org/graphql 失败: HTTP 422 (Cannot query field "findScenes" on type "Query")',
    )
    ctx = Context(input=CrawlerInput.empty())

    with pytest.raises(CrawlerException) as exc_info:
        await crawler._post_graphql(ctx, "query { }", {}, operation="番号检索")

    msg = str(exc_info.value)
    assert "[番号检索]" in msg
    assert "HTTP 422" in msg
    assert 'Cannot query field "findScenes"' in msg


# [JavStash] 测试候选词回退（如加勒比无码 CAPPV-081817_001 检索首词未中，自动尝试候选 081817_001 并成功）
@pytest.mark.asyncio
async def test_javstash_fallback_candidates(mock_client):
    manager.config.javstash_api_key = "test_key"
    crawler = StashGraphQLCrawler(client=mock_client)

    mock_client.post_json.side_effect = [
        ({"data": {"queryScenes": {"scenes": []}}}, None),
        (
            {"data": {"queryScenes": {"scenes": [{"id": "scene-1", "title": "Test Caribbean", "code": "081817_001"}]}}},
            None,
        ),
    ]
    inp = CrawlerInput.empty()
    inp.number = "CAPPV-081817_001"
    ctx = Context(input=inp)

    res = await crawler._run(ctx)
    assert res.title == "Test Caribbean"
    assert res.number == "081817_001"
    assert mock_client.post_json.call_count == 2


@pytest.mark.asyncio
async def test_javstash_search_by_code_modifiers(mock_client):
    manager.config.javstash_api_key = "test_key"
    crawler = StashGraphQLCrawler(client=mock_client)

    # 1. EQUALS returns empty, INCLUDES returns match
    mock_client.post_json.side_effect = [
        ({"data": {"queryScenes": {"scenes": []}}}, None),
        (
            {
                "data": {
                    "queryScenes": {
                        "scenes": [{"id": "scene-carib", "title": "Carib Match", "code": "051726-001-carib"}]
                    }
                }
            },
            None,
    ...
    inp = CrawlerInput.empty()
    inp.number = "051726-001"
    ctx = Context(input=inp)

    res = await crawler._run(ctx)
    assert res.title == "Carib Match"
    assert res.number == "051726-001-carib"
    assert mock_client.post_json.call_count == 2
    # Verify the second call used INCLUDES modifier
    call_vars = mock_client.post_json.call_args_list[1][1]["json_data"]["variables"]
    assert call_vars["input"]["code"]["modifier"] == "INCLUDES"
    assert call_vars["input"]["code"]["value"] == "051726-001"


@pytest.mark.asyncio
async def test_javstash_search_fallback_to_text(mock_client):
    manager.config.javstash_api_key = "test_key"
    crawler = StashGraphQLCrawler(client=mock_client)

    # EQUALS empty, INCLUDES empty, text search matches
    mock_client.post_json.side_effect = [
        ({"data": {"queryScenes": {"scenes": []}}}, None),
        ({"data": {"queryScenes": {"scenes": []}}}, None),
        (
            {"data": {"queryScenes": {"scenes": [{"id": "scene-text", "title": "Text Match", "code": "TEXT-001"}]}}},
            None,
        ),
    ]
    inp = CrawlerInput.empty()
    ...
    res = await crawler._run(ctx)
    assert res.title == "Text Match"
    assert res.number == "TEXT-001"
    assert mock_client.post_json.call_count == 3
    call_vars = mock_client.post_json.call_args_list[2][1]["json_data"]["variables"]
    assert call_vars["input"] == {"text": "TEXT-001"}


def test_javstash_boundary_match():
    from mdcx.crawlers.javstash import check_boundary_match

    assert check_boundary_match("SSIS-123", "SSIS-123") is True
    assert check_boundary_match("SSIS-123-C", "SSIS-123") is True
    assert check_boundary_match("SSIS-123_01", "SSIS-123") is True
    assert check_boundary_match("SSIS-1234", "SSIS-123") is False
    assert check_boundary_match("1SSIS-123", "SSIS-123") is False
    assert check_boundary_match("", "SSIS-123") is False
    assert check_boundary_match("SSIS-123", "") is False


def test_javstash_disambiguation_1pon_vs_carib():
    from mdcx.crawlers.javstash import select_best_scene

    carib_scene = {
        "id": "carib-1",
        "title": "Caribbean Scene",
        "code": "011123-001-carib",
        "studio": {"name": "Caribbeancom"},
        "urls": [{"url": "https://www.caribbeancom.com/moviepages/011123-001/index.html"}],
    }
    pondo_scene = {
        "id": "1pon-1",
        "title": "一本道ベスト2022 ～ パート1 ～",
        "code": "011123_001-1pon",
        "studio": {"name": "一本道"},
        "urls": [{"url": "https://www.1pondo.tv/movies/011123_001/"}],
    }

    # For 011123_001 (1Pondo date number format with underscore)
    best = select_best_scene([carib_scene, pondo_scene], "011123_001", original_number="011123_001")
    assert best == pondo_scene

    # Even if carib_scene is first in list, 1Pondo should win decisively
    best_inverted = select_best_scene([carib_scene, pondo_scene], "011123_001")
    assert best_inverted == pondo_scene

    # For 011123-001 (Caribbeancom format with hyphen)
    best_carib = select_best_scene([pondo_scene, carib_scene], "011123-001", original_number="011123-001")
    assert best_carib == carib_scene


@pytest.mark.asyncio
async def test_javstash_uncensored_multi_scene_selection(mock_client):
    manager.config.javstash_api_key = "test_key"
    crawler = StashGraphQLCrawler(client=mock_client)
    ...
    carib_scene = {
        ...
        "code": "011123-001-carib",
        "studio": {"name": "Caribbeancom"},
        "performers": [{"performer": {"name": "西内萌菜", "gender": "FEMALE"}}],
        "urls": [{"url": "https://www.caribbeancom.com/moviepages/011123-001/index.html"}],
    }
    pondo_scene = {
        "id": "1pon-1",
        ...
        "studio": {"name": "一本道"},
        "performers": [
            {"performer": {"name": "百多えみり", "gender": "FEMALE"}},
            {"performer": {"name": "穂高ひな", "gender": "FEMALE"}},
        ],
        "urls": [{"url": "https://www.1pondo.tv/movies/011123_001/"}],
    }

    # Stash-box returns both scenes from search
    mock_client.post_json.side_effect = [
        ({"data": {"queryScenes": {"scenes": [carib_scene, pondo_scene]}}}, None),
    ]
    inp = CrawlerInput.empty()
    inp.number = "011123_001-1pon"
    ctx = Context(input=inp)

    res = await crawler._run(ctx)
    assert res.title == "一本道ベスト2022 ～ パート1 ～"
    assert res.studio == "一本道"
    assert "百多えみり" in res.actors
