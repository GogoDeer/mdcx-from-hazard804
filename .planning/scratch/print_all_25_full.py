...
# Find all completed movie blocks
# Pattern: 🕷 xx:xx:xx (xx/25) (filename) 刮削完成！
completed_blocks = re.findall(r"🕷 \d+:\d+:\d+ (\d+/25) (.*?) 刮削完成！用时 ([\d\.]+) 秒！", content)
print(f"Total completed: {len(completed_blocks)}")

# Let's find each movie by searching for '🙈 [file]'
file_blocks = re.split(r"(?=🙈 \[file\])", content)[1:]
print(f"Total file blocks: {len(file_blocks)}")
...
for i, b in enumerate(file_blocks, 1):
    ...
    # originaltitle
    otitle_m = re.search(r"originaltitle:\s*(.*)", b)
    otitle = otitle_m.group(1).strip() if otitle_m else ""
